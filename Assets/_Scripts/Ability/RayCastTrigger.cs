...
public class RayCastTrigger : MonoBehaviour {
    ...
    [HideInInspector]
    public float abilityRange = 50f;                   // Distance in unity units over which the player can fire.
    [HideInInspector]
    ...
    public void Fire()
    {
        ...
        Debug.DrawRay(rayOrigin, cam.transform.forward * abilityRange, Color.green);
        ...
        if (Physics.Raycast(rayOrigin, cam.transform.forward, out hit, abilityRange))
        {
        ...
        else
        {
            ...
            laserLine.SetPosition(1, cam.transform.forward * abilityRange);
        }
    }
